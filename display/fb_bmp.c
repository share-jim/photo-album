...
static int fb_bmp_Resolve2(struct pic_info *pPic)
{
	int fd = -1; 
	unsigned long len =0;
...
int display_bmp(const char *filename)
{
	...
	//3.显示bmp
	//fb_draw_picture_bmp(&picture);   //显示有效数据,倒立镜像
	//fb_draw_picture_bmp2(&picture);   //显示有效数据，镜像
	fb_draw_picture_bmp3(&picture);  	////显示有效数据，原始图像
	return 0;
