...
static int fb_png_Resolve2(struct pic_info *pPic)
{
	...
	FILE *file=NULL;
	int color_type;
	png_bytep* row_pointers;  //unsigned char *
	int i=0,j=0;
	unsigned long len=0;
	int pos=0;
	...
	//1.相关数据结构体的初始化
     ...
     png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, 0, 0, 0); //朱
   ...
   //3.绑定文件指针与解码器
	...
	png_read_png(png_ptr, info_ptr, PNG_TRANSFORM_EXPAND | PNG_TRANSFORM_STRIP_ALPHA, 0); //朱/* read only */
	...
	//6.获取真正的图像信息
	row_pointers = png_get_rows(png_ptr,info_ptr);         //朱 (我：指针的内存申请在4.)
